...
### control-room/control-room normalization preference

For control-room or dense company-control-room UI, normalization must be visually verified, not code-only. Preserve the current tone/colors unless a redesign is explicitly requested. Improvements should focus on system rules: icon size/stroke consistency, row/card rhythm, panel width balance, spacing scale, text baseline/metadata wrapping, chip/button alignment, and auto-layout behavior. Avoid adding gradients, decorative color changes, or generic dashboard styling as a substitute for consistency.
