...
Layout and rhythm work must be validated on the rendered UI, not inferred from code alone. When the user asks for system consistency, auto-layout feel, icon uniformity, layout uniformity, card rhythm, or says “don’t just look at the code,” run a preview/screenshot/vision QA loop before and after edits.

- Preserve existing tone, palette, and brand treatment unless the user explicitly asks for a visual concept change.
- Do not “fix” structural rhythm by adding gradients, new colors, decorative accents, or novelty.
- Check actual rendered balance: sidebar/main/right-panel weight, component baselines, icon size and stroke, row heights, card gaps, control heights, meta text wrapping, truncation, and typography rhythm.
- Use the smallest spacing/layout/component-system patch that improves consistency, then visually verify again.
