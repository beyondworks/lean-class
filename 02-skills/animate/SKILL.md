...
## Reference-Driven Motion Adaptation

When the user provides a motion/design reference URL, especially Dribbble-style animation shots, do not just describe it. Capture the actual media when possible, generate a contact sheet, extract reusable motion/design principles, and adapt those principles into the user's context without copying the shot. For data-card/security-style references, follow.

Key rule: render → contact sheet → full-frame QA → patch layout/legibility bugs → re-render before delivery. Card/list clipping and too-small labels are delivery blockers, not notes for later.
